...
class DistributedInferenceService:
    ...
    async def submit_task_async(self, task_data: dict) -> Optional[dict]:
        ...
        # Only rank0 handles requests; others receive via broadcast.
    ...
    def server_metadata(self):
        assert hasattr(self, "args"), "Inference service not started"
        return {
            "nproc_per_node": self.worker.world_size,
            "ckpt_dir": getattr(self.args, "ckpt_dir", None),
            "wav2vec_dir": getattr(self.args, "wav2vec_dir", None),
            "infinitetalk_dir": getattr(self.args, "infinitetalk_dir", None),
            "size": getattr(self.args, "size", None),
        }
