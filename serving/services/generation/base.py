...
class BaseGenerationService(ABC):
    ...
    async def _process_image_path(self, image_path: str, task_data: Dict[str, Any]) -> None:
        """
        In InfiniteTalk serving, `image_path` can be either an image OR a video.
        We keep the field name for LightX2V compatibility.
        """
        if not image_path:
            return
    ...
    async def _process_talk_objects(self, talk_objects: list, task_data: Dict[str, Any]) -> None:
        # Keep the hook for API compatibility; InfiniteTalk serving currently does not use talk_objects.
        if not talk_objects:
            return
    ...
    async def generate_with_stop_event(self, message: Any, stop_event) -> Optional[Any]:
        try:
            ...
            if hasattr(message, "image_path") and message.image_path:
                ...
                logger.info(f"Task {message.task_id} media path: {task_data.get('image_path')}")
            ...
            task_data["resize_mode"] = getattr(message, "resize_mode", "adaptive")
            ...
            if result.get("status") == "success":
                ...
                if not actual_save_path.suffix:
                    actual_save_path = actual_save_path.with_suffix(self.get_output_extension())
                return TaskResponse(task_id=message.task_id, task_status="completed", save_result_path=actual_save_path.name)
