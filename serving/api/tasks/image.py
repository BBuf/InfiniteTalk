...
async def create_image_task(message: ImageTaskRequest):
    ...
    InfiniteTalk is a video generation model; this endpoint is provided but will return 400 unless
    the underlying worker is configured to support image tasks (not implemented).
    """
    try:
        ...
        raise HTTPException(status_code=400, detail="Image tasks are not supported by InfiniteTalk serving. Use /v1/tasks/video.")
...
async def create_image_task_form(
    ...
    # Keep compatibility: create task then fail early (clear message).
